import { NotFoundException } from '@nestjs/common';
import { GetOrderByNumberUseCase } from './get-order-by-number.usecase';
import type { OrderRepositoryPort } from './ports/order-repository.port';
import type { Order } from '../domain/order';

describe('GetOrderByNumberUseCase', () => {
  const SAMPLE_ORDER: Order = {
    id: 'order-1',
    orderNumber: 'ORD-2026-0001',
    status: 'PEDIDO',
    phase: 'ABERTO',
    clientId: 'client-1',
    subtotal: 500,
    discount: 0,
    shippingCost: 0,
    taxAmount: 0,
    totalAmount: 500,
    orderDate: new Date(),
    items: [],
    createdAt: new Date(),
    updatedAt: new Date(),
  };

  it('retorna pedido quando encontrado pelo número', async () => {
    const findMock = jest.fn().mockResolvedValue(SAMPLE_ORDER);
    const repo: OrderRepositoryPort = {
      findByOrderNumber: findMock,
      findById: jest.fn(),
      findMany: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      cancel: jest.fn(),
      delete: jest.fn(),
      getStats: jest.fn(),
      generateNextOrderNumber: jest.fn(),
    };

    const sut = new GetOrderByNumberUseCase(repo);
    const result = await sut.execute('ORD-2026-0001');
    expect(result.orderNumber).toBe('ORD-2026-0001');
  });

  it('lança NotFoundException se não existir', async () => {
    const findMock = jest.fn().mockResolvedValue(null);
    const repo: OrderRepositoryPort = {
      findByOrderNumber: findMock,
      findById: jest.fn(),
      findMany: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      cancel: jest.fn(),
      delete: jest.fn(),
      getStats: jest.fn(),
      generateNextOrderNumber: jest.fn(),
    };

    const sut = new GetOrderByNumberUseCase(repo);
    await expect(sut.execute('ORD-GHOST')).rejects.toThrow(NotFoundException);
  });
});
